...
#To use this program you need to install numpy, jax, onnx, onnxruntime, jaxonnxruntime (with pip)
...
#Modification of the exponential fit, can be changed
def _fit_exponential_ratio(x_coord, ti_over_te):
    """Fits y=a*exp(bx) with Gauss-Newton least squares in JAX."""

    x = jnp.asarray(x_coord)
    y = jnp.clip(jnp.asarray(ti_over_te), 1e-12)

    init = jnp.asarray([1.0, 1.0], dtype=x.dtype)
    damping = jnp.asarray(1e-8, dtype=x.dtype)
    ...
    def _step(_, params):
        ...
        # Jacobian of a*exp(bx) wrt [a, b]
        jac = jnp.stack((exp_bx, a * x * exp_bx), axis=1)
        jtj = jac.T @ jac + damping * eye2
        jtr = jac.T @ residual
        delta = jnp.linalg.solve(jtj, jtr)

        updated = params - delta

        updated = updated.at[0].set(jnp.clip(updated[0], 1e-12))

        all_finite = jnp.all(jnp.isfinite(updated))
        return jnp.where(all_finite, updated, params)

    return jax.lax.fori_loop(0, 20, _step, init)
...
def evaluate_model( pellet_radius, vel_value, x_coord, Te, ne, Ti, q, B0, first_point=[1.8, 0.47], second_point=[2.6192, -0.136],inj_value=None):
    ...
    if inj_value in ('WEST_upHFS', "WEST_midHFS","WEST_lowHFS","WEST_LFS"):
        ...
        norm = jnp.load(SCALERS_PATH / "WEST" / "Normalization.npz")
    ...
    # Exponential fit for Ti/Te (JAX-compatible approximation)
